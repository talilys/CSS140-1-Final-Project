...
EMOTICON_MAP = [
    ...
    (r"(?<!\w)x[D3]+(?!\w)",        "laughing very happy"),
    ...
    (r"(?<!\w)[uU]_+[uU](?!\w)",    "sad defeated down"),

    # ── T_T family — handles T_T, T__T, TT, T ___ T, t.t, etc. ──
    (r"(?<!\w)[Tt]\s*[_.]{1,}\s*[Tt](?!\w)", "crying devastated sad"),
    (r"(?<!\w)[Tt]{2,}(?!\w)",       "crying sad"),
    (r"(?<!\w)[Qq]\s*[_.]{1,}\s*[Qq](?!\w)", "crying sad upset"),

    # ── Shock / surprise ──
    (r"(?<!\w):-?[oO0]+(?!\w)",      "shocked surprised"),
    (r"(?<!\w)\*_\*(?!\w)",          "amazed excited starstruck"),

    # ── Neutral / deadpan / playful ──
    (r"(?<!\w):-?\|+(?!\w)",         "neutral expressionless"),
    (r"(?<!\w):-?[pP]+(?!\w)",       "playful joking"),

    # ── Text-based laughs / sighs (now support repeated letters) ──
    (r"(?<!\w)lo+l(?!\w)",           "laughing funny"),
    (r"(?<!\w)lma+o+(?!\w)",         "laughing very funny"),
    (r"(?<!\w)lmfa+o+(?!\w)",        "laughing hilarious"),
    (r"(?<!\w)ha(?:ha)+(?!\w)",      "laughing happy"),
    (r"(?<!\w)he(?:he)+(?!\w)",      "laughing playful"),
    (r"(?<!\w)hu(?:hu)+(?!\w)",      "crying sad upset"),
    (r"(?<!\w)hays?t?(?!\w)",        "disappointed tired resigned"),
    (r"(?<!\w)ay+(?!\w)",            "disappointment sigh"),
    (r"(?<!\w)om+g+(?!\w)",          "shocked overwhelmed surprised"),
    (r"(?<!\w)omf+g+(?!\w)",         "shocked very overwhelmed"),
    (r"(?<!\w)ugh+(?!\w)",           "frustrated annoyed"),
    (r"(?<!\w)argh+(?!\w)",          "frustrated angry"),
    (r"(?<!\w)no+o+(?!\w)",          "devastated upset"),
    (r"(?<!\w)ye+s+(?!\w)",          "excited very happy"),
    (r"(?<!\w)ya+y+(?!\w)",          "excited happy celebrating"),
]
